...
#pragma glslify: noise = require('./octave-simplex.glsl');

void main() {
  float amp = 20.;
  vec2 dx = vec2(.01, 0.);
  vec2 dz = vec2(0., .01);
  vec3 vx = vec3(a_PlaneVertex.x + .01, amp * noise(a_PlaneVertex + dx), a_PlaneVertex.y);
  vec3 vz = vec3(a_PlaneVertex.x, amp * noise(a_PlaneVertex + dz), a_PlaneVertex.y + .01);
